...
public class GameBoard1 extends JPanel {
	...
	public GameBoard1() throws IOException {
		isStopped = false;
		...
		URL clipUrl = getClass().getResource("sound/50_sniper_shot-Liam-2028603980.wav");
		if(clipUrl == null) {
...
//			icon[i] = new ImageIcon(getClass().getResource(iconFiles[i]));
	...
	public Thread generator() {
		...
		for(int i = 0; i < 8; ++i)
			add(generator.generateNewEnemy1());
		...
		return new Thread(new Runnable() {	
			...
			public void run() {
				...
				while(!isGameOver && !isStopped) {
					...
					if(!isGameOver)
						add(generator.generateNewEnemy1());
					if(s > Game.REFRESH_TIME - 30)
						s -= 8;
					else
						s = Game.REFRESH_TIME + 2;
				}
			}});
	}
	...
	public void gameOver() {
		...
		case JOptionPane.YES_OPTION:
			Game.setState(GameState.CONTINUE1);
			break;

		case JOptionPane.NO_OPTION:
			Game.setState(GameState.OVER1);
			break;
		}
	}
